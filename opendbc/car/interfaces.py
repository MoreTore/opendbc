...
class CarInterfaceBase(ABC):
  ...
  def configure_torque_tune(candidate: str, tune: structs.CarParams.LateralTuning, steering_angle_deadzone_deg: float = 0.0, use_steering_angle: bool = True):
    ...
    tune.torque.sigmoidSharpness = params['SIGMOID_SHARPNESS'] if not np.isnan(params['SIGMOID_SHARPNESS']) else 0.0
    tune.torque.sigmoidTorqueGain = params['SIGMOID_TORQUE_GAIN'] if not np.isnan(params['SIGMOID_TORQUE_GAIN']) else 0.0
